document.addEventListener('DOMContentLoaded', function() {
  ...
  function drawOnCanvasForProcessing(canvas, baseImage, callback) {
    ...
    if (preloadedLogo) {
      const logoSettings = {
        size_percent: logoSizeSlider.value,
        opacity: logoOpacitySlider.value,
        position: logoPositionSelect.value
      };
      ...
      const positions = {
        ...
        "Top-Right": [canvas.width - new_w - margin, margin],
        "Bottom-Left": [margin, canvas.height - new_h - margin],
        "Bottom-Right": [canvas.width - new_w - margin, canvas.height - new_h - margin],
  ...
  // --- وظيفة بدء المعالجة (تفرّق بين كمبيوتر / أندرويد / iOS) ---
  async function startProcessing() {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      drawOnCanvasForProcessing(tempCanvas, baseImage, (canvas) => {
        ...
        if (mobile) {
          if (isIOS()) {
            // iOS: تحويل الـ dataURL إلى Blob ثم فتحه في نفس الصفحة
            fetch(dataUrl)
              .then(res => res.blob())
              .then(blob => {
                const blobUrl = URL.createObjectURL(blob);
                // فتح الصورة في نفس الصفحة؛ المستخدم يحفظها بالضغط المطوّل
                window.location.href = blobUrl;
              });
          } else {
            // أندرويد: تحميل مباشر (يعمل جيدًا كما ذكرت)
  ...
  function loadSettings(){
    ...
    if(savedSettings){
      ...
      logoPositionSelect.value = settings.logoPosition;
      templateOpacitySlider.value = settings.templateOpacity;
      document.querySelector(`input[name="imageSizeMode"][value="${settings.imageSizeMode}"]`).checked = true;
      document.getElementById("logoSizeValue").textContent = settings.logoSize;
      document.getElementById("logoOpacityValue").textContent = settings.logoOpacity;
      document.getElementById("templateOpacityValue").textContent = settings.templateOpacity;
    }
  }

  // --- إعادة تعيين الشعار والقالب والكل ---
  function resetLogo() {
    preloadedLogo = null;
    logoInput.value = "";
    ...
    if(imageFiles.length > 0){
      ...
      (()=>{
        ...
        reader.onload = (ev=>{
          ...
          preloadedBaseImage.src = ev.target.result;
        });
    ...
    if(logoFile){
      ...
      (()=>{
        ...
        reader.onload = (ev=>{
          ...
          preloadedLogo.onload = updateLivePreview;
          preloadedLogo.src = ev.target.result;
        });
  ...
  templateInput.addEventListener('change',(e)=>{
    ...
    if(templateFile){
      ...
      (()=>{
        const reader = new FileReader();
        reader.onload = (ev=>{
          preloadedTemplate = new Image();
          preloadedTemplate.onload = updateLivePreview;
          preloadedTemplate.src = ev.target.result;
        });
  ...
  document.querySelectorAll('input[type="range"], select, input[type="radio"]').forEach((el=>{
    el.addEventListener("input",(()=>{
      if("range" === el.type){
        const spanVal = document.getElementById(el.id + "Value");
        if(spanVal) spanVal.textContent = el.value;
      }
      updateLivePreview();
      saveSettings();
    }));
